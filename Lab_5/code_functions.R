...
summary_nona(x, mean, trim = 0.1)
# Default parameters:
summary_nona = function(x, func = mean, ...) {
    func(x, na.rm = TRUE, ...)
}
summary_nona(x, trim = 0.1)
...
summary_nona_list2 = function(l, functions_list, ...) {
    lapply(l, function(x) lapply(functions_list, function(func) func(x, na.rm = TRUE, ...))) # notice the anonymous function
}
...
# Time to talk about environments
